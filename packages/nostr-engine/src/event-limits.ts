...
export function validateEventTemplate(template: EventTemplate): void {
  ...
  if (typeof template.content !== "string" || byteLength(template.content) > MAX_EVENT_CONTENT_BYTES) throw new Error("invalid-event");
  ...
  for (const tag of template.tags) {
    ...
    values += tag.length;
    if (values > MAX_EVENT_TAG_VALUES || tag.some((value) => typeof value !== "string" || byteLength(value) > MAX_EVENT_TAG_VALUE_BYTES)) throw new Error("invalid-event");
  }
}
