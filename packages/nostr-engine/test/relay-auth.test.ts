...
describe("relay authentication", () => {
  it("uses current account once for each relay challenge and tears down", async () => {
    ...
    const relay = { url: "wss://relay.example/", challenge$, connected$: new BehaviorSubject(false), attempts$: new BehaviorSubject(0), get challenge() { return challenge$.value; }, authenticate };
    ...
    const authenticator = createRelayAuthenticator(pool as never, accounts, undefined, telemetry);
    ...
    await vi.waitFor(() => expect(telemetry.snapshot()).toContainEqual(expect.objectContaining({ name: "relay.authentication", value: 1 })));
