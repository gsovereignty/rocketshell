...
import { DEFAULT_PUBLISH_TIMEOUT_MS } from "@platform/nostr-engine";
...
export function createRelayPoolLike(pool: RelayPool): RelayPoolLike {
  return {
    ...
    async publish(relayUrls, event) {
      const outcomes = await pool.publish(relayUrls, event as NostrEvent, { retries: false, timeout: DEFAULT_PUBLISH_TIMEOUT_MS });
