...
## Step 1.5 - Triage The Workspace

This repository already owns napplet build plumbing. Inspect the target and a
current sibling napplet before editing:
...
For an empty target, mirror build/test structure and current dependency versions
from a sibling napplet. For an existing target, preserve compatible substrate and
edit only product surfaces. Build and test directly through workspace scripts.
...
3. Run `build-napplet` against that spec, preserving workspace build scripts and
   manifest structure while editing project-specific files
...
passes workspace validation (`pnpm verify`, `pnpm test:conformance`), any
