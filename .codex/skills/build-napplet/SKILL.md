...
## Step 1 — Use The Workspace Substrate

This repository keeps napplets in one pnpm workspace. Inspect a current sibling
napplet, then preserve its package scripts, Vite single-file setup, TypeScript
config, dependency versions, and conformance wiring. New empty target directories
mirror that substrate directly.

## Step 2 — Project-Specific Edit Points

Keep workspace-owned files and make only project-specific edits:
...
Sibling napplets already wire `nip5aManifest`. Update hard `requires` rather
than replacing config. `nip5aManifest` hashes build output and writes a local
sidecar. `nappletType` is manifest d-tag metadata.
...
The aggregate hash is computed into `.nip5a-manifest.json`; it is not injected
as a meta tag. Builds remain unsigned locally.
...
Before claiming done, run workspace-standard commands from target directory:
...
Run feature-specific unit tests through `pnpm verify`.

## Common pitfalls

- Diverging from sibling workspace tooling — **wrong substrate.** Mirror current
  scripts, config shape, versions, and layout.
