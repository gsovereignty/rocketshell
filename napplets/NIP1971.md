...
| `MUST`      | Status is `claimed`                                | `["claim", "<claim-event-id>", "<claimant>"]`                               |
...
- the owner; or
- a maintainer listed by each referenced previous revision; or
- the owner of an ancestor of the problem in the problem DAG of each referenced
  previous revision.

An ancestor is any problem reachable by following one or more direct-parent
edges. Direct parents are ancestors. Ancestor authorization therefore extends
from the problem's direct parents through the graph root.

To validate an ancestor owner, clients MUST resolve the direct-parent
coordinates from each referenced previous revision, then recursively resolve
the selected current revision of every reachable parent. Clients MUST NOT infer
ancestor authorization from the graph-root `A/E/K/P` tags alone. If required
parent revisions are unavailable, or any reachable parent has unresolved
revision forks, ancestor authorization is not established.

Only the owner MAY add or remove maintainers.

Every owner-authored revision MUST list the owner and every resolved ancestor
owner as maintainers. Existing maintainers remain listed unless the owner
removes them.

Maintainer-authored revisions MUST copy the previous maintainer list unchanged.

Ancestor-owner-authored revisions MUST copy the previous maintainer list
unchanged.

Multiple revisions referencing the same previous event are forks.

An eligible head is a revision-DAG head whose author is one of:

- the problem owner from the origin coordinate;
- a maintainer named by a `p` tag marked `maintainer`; or
- a logical maintainer named by an unmarked direct-parent-owner `p` tag.

When eligible heads have different `created_at` values, clients MUST select the
eligible head with the greatest `created_at` as the current head. This rule
applies even when the selected head does not reference every older head or an
intermediate revision is unavailable.

When two or more eligible heads share the greatest `created_at`, the current
head is unresolved until a later eligible head or merge revision supersedes the
tie.

Heads whose authors are not eligible MUST NOT become current based on
`created_at`.

Clients MUST retain every known revision, including every previous, current,
competing, and ineligible head. Selecting a current head or receiving a later
revision that references an older head MUST NOT delete or discard the older
revision. The `previous` links preserve the complete revision DAG for history,
audit, authorization, and later merges.

Current-head selection affects only which complete snapshot represents current
problem state. It does not alter revision retention or revision-DAG membership.

Clients MUST NOT use event ID, relay arrival order, or any other tie-breaker to
resolve equal greatest `created_at` values.
...
Ancestor traversal follows these selected current parent edges recursively.
Clients MUST track visited problem coordinates while traversing and MUST reject
a traversal that encounters a cycle. A problem reachable through multiple
paths is one ancestor and grants its owner one authorization role.
...
| Workflow event | Reflected in a later kind `31971` revision                              |
| -------------- | ----------------------------------------------------------------------- |
| `["claim"]`    | `["status", "claimed"]` and `["claim", "<event-id>", "<claimant>"]` |
| `["patched"]`  | `["status", "patched"]` and `["patch", "<event-id>", "<solution-author>"]` |

### Best-effort claims

A claim event is valid when it references the selected current problem revision
and its `created_at` is not earlier than that revision's `created_at`.

For every status except `rfm`, a valid claim immediately establishes a
best-effort claim. The claimant MAY begin work and submit a pull request without
waiting for the problem author or a maintainer to publish another kind `31971`
revision.

The claim lasts for 24 hours, beginning at the claim event's `created_at` and
ending at `created_at + 86400`. After expiry, clients SHOULD treat the problem
as available for another claim unless a later problem revision changes its
status. Among competing unexpired claims for the same revision, clients SHOULD
select the claim with the earliest `created_at`, breaking equal-timestamp ties
by the claim event ID in ascending lexical order.

The problem author or a maintainer SHOULD publish a new kind `31971` revision
with status `claimed` and the corresponding `claim` tag when the claimant is in
their Web of Trust. This revision records their acknowledgement but does not
gate work or pull-request submission.

When the selected problem status is `rfm`, a claim event is only a request. Work
SHOULD wait until the problem author or a maintainer publishes a kind `31971`
revision that changes the status to `claimed` and references that claim event.

If the claimant does not submit a pull request before the 24-hour claim expires,
the problem author or a maintainer SHOULD publish a new `open` revision without
the `claim` tag when the selected revision still records that claim.
...
| `rfm`      | Request for maintainers | `draft`, `big`, `open`, `claimed`, `closed`       |
