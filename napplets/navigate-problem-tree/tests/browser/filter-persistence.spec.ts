...
test("merit request action is absent", async ({ page }) => {
  await page.addInitScript((queryEvents) => {
  ...
  await page.goto("/");
  await page.locator(`.tree-node[data-select="${firstParent}"]`).click();
  await expect(page.getByRole("button", { name: "SUBMIT MERIT REQUEST" })).toHaveCount(0);
  await expect.poll(() => page.evaluate(() => (window as Window & { __intentInvocations: unknown[] }).__intentInvocations)).toEqual([]);
});
